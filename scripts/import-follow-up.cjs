...
async function main() {
  ...
  // Connect to DB
  const dbUrl = process.env.DATABASE_URL || 'postgresql://importacao:importacao123@localhost:5432/importacao';
  ...
  for (const row of rows) {
    ...
    if (parseNumeric(row[COL.customsValue])) extraData.customsValue = parseNumeric(row[COL.customsValue]);
    if (excelDateToISO(row[COL.cashPaymentDate])) extraData.cashPaymentDate = excelDateToISO(row[COL.cashPaymentDate]);
    ...
    if (cleanString(row[COL.dispatcherRef])) extraData.dispatcherRef = cleanString(row[COL.dispatcherRef], 100);
    if (cleanString(row[COL.connections])) extraData.connections = cleanString(row[COL.connections]);
    if (cleanString(row[COL.transshipPort])) extraData.transshipPort = cleanString(row[COL.transshipPort]);
    if (cleanString(row[COL.voyageMain])) extraData.voyageMain = cleanString(row[COL.voyageMain]);
    if (cleanString(row[COL.voyageConnection])) extraData.voyageConnection = cleanString(row[COL.voyageConnection]);
    ...
    // Correction data
    const correctionNeeded = String(row[COL.docCorrection] || '').toUpperCase().includes('SIM');
    ...
    try {
      ...
      if (existingId) {
        ...
        `, [
          ...
          purchaseRef, vesselName, blNumber,
          shippingLine, insuranceValue, consolidationRef,
          etaCarrier, etaActual, containerCount,
          freightAgent, originCity, inspectionType,
          diNumber, customsChannel, customsClearanceAt,
          cdArrivalAt, freeTimeDays,
          numerarioValue, numerarioPct,
        ]);
        ...
        `, [
          ...
          urgency || null,
          savedToFolderAt, ncmBlCheckedAt, freightBlCheckedAt, espelhoBuiltAt,
          invoiceSentFeniciaAt, signaturesCollectedAt, signedDocsSentAt, diDraftAt,
        ]);
        ...
        `, [
          ...
          status === 'completed' ? 100 : status === 'validated' ? 60 : status === 'documents_received' ? 30 : 10,
          urgency || null,
          savedToFolderAt, ncmBlCheckedAt, freightBlCheckedAt,
          espelhoBuiltAt, invoiceSentFeniciaAt, signaturesCollectedAt,
          signedDocsSentAt, diDraftAt,
        ]);
      ...
      if (correctionNeeded || (errorCount && errorCount > 0)) {
        ...
        const errorTypes = errorTypeRaw
          ? JSON.stringify(errorTypeRaw.split(/[,;\/]/).map(s => s.trim()).filter(Boolean))
          : null;
