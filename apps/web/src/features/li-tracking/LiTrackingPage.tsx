...
import { FileCheck, Search, ChevronDown, ChevronLeft, ChevronRight, Clock, CheckCircle2, Send, XCircle } from 'lucide-react';
...
const statusConfig: Record<string, { bg: string; text: string; dot: string; label: string }> = {
  deferred: { bg: 'bg-emerald-50', text: 'text-emerald-700', dot: 'bg-emerald-500', label: 'Deferida' },
...
const defaultStatus = { bg: 'bg-slate-100', text: 'text-slate-700', dot: 'bg-slate-400', label: '' };
...
const kpiList = [
  ...
  { key: 'deferred', label: 'Deferidas', icon: CheckCircle2, gradient: 'from-emerald-500 to-emerald-600' },
...
export function LiTrackingPage() {
  ...
  const { data: liResponse, isLoading } = useApiQuery<{ data: LiItem[]; pagination: { page: number; totalPages: number; total: number } }>(
  ...
  const { data: stats } = useApiQuery<LiStats>(
    ['li-tracking-stats'],
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500">
            {pagination?.total ?? liData?.length ?? 0} licenca{(pagination?.total ?? liData?.length ?? 0) !== 1 ? 's' : ''} registrada{(pagination?.total ?? liData?.length ?? 0) !== 1 ? 's' : ''}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {kpiList.map((kpi) => (
          <div key={kpi.key} className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-3.5">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-slate-900">{stats?.byStatus?.[kpi.key] ?? 0}</p>
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
          ...
          <div className="max-w-[200px]">
            ...
            <div className="relative">
              <select
                value={orgaoFilter}
                onChange={(e) => { setOrgaoFilter(e.target.value); setPage(1); }}
                className="w-full appearance-none rounded-xl border border-slate-200 py-2.5 pl-3.5 pr-10 text-sm text-slate-900 transition-colors focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20"
              >
            ...
            <table className="min-w-full divide-y divide-slate-200">
              <thead>
                <tr className="bg-slate-50/80">
                  ...
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Orgao</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">NCM</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Status</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Solicitacao</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Deferimento</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Validade</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">LPCO</th>
                </tr>
              </thead>
                ...
                {liData.map((li) => (
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap px-5 py-3.5 text-sm text-slate-500">
                      {li.requestedByCompanyAt ? formatDate(li.requestedByCompanyAt) : <span className="text-slate-300">--</span>}
                    </td>
                    <td className="whitespace-nowrap px-5 py-3.5 text-sm text-slate-500">
                      {li.deferredAt ? formatDate(li.deferredAt) : li.expectedDeferralAt ? (
                        <span className="text-amber-600">Prev. {formatDate(li.expectedDeferralAt)}</span>
                      ) : <span className="text-slate-300">--</span>}
                    </td>
                    <td className="whitespace-nowrap px-5 py-3.5 text-sm text-slate-500">
                      {li.validUntil ? formatDate(li.validUntil) : <span className="text-slate-300">--</span>}
                    </td>
                    <td className="whitespace-nowrap px-5 py-3.5 text-sm font-mono text-slate-700">
