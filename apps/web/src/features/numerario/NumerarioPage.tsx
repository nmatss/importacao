...
function getNumericField(data: Record<string, unknown> | null | undefined, key: string): number | null {
  if (!data) return null;
  const val = data[key];
  if (val == null) return null;
...
export function NumerarioPage() {
  ...
  const numerarioProcesses = useMemo(() => {
    ...
    return allProcesses.filter((p) => {
      ...
      return data.valorNumerario != null || data.dataPgtoNumerario != null || data.percentualNumerario != null;
    });
  }, [allProcesses]);
  ...
  const kpiCards = [
    { label: 'Total Numerario', value: formatCurrency(totalNumerario, 'BRL'), icon: DollarSign, gradient: 'from-emerald-500 to-emerald-600' },
    { label: 'Processos', value: numerarioProcesses.length, icon: Calculator, gradient: 'from-blue-500 to-blue-600' },
    { label: 'Percentual Medio', value: `${avgPercentage}%`, icon: Percent, gradient: 'from-violet-500 to-violet-600' },
  ];
            ...
            <table className="min-w-full divide-y divide-slate-200">
              <thead>
                <tr className="bg-slate-50/80">
                  ...
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Valor FOB</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Valor Numerario</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">% Numerario</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Data Pagamento</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Solicitante</th>
                  <th className="px-5 py-3.5 text-left text-xs font-semibold uppercase tracking-wider text-slate-500">Dados Cambio</th>
                </tr>
              </thead>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="whitespace-nowrap px-5 py-3.5 text-sm font-medium text-slate-900">
                        {fobValue != null && !isNaN(fobValue) ? formatCurrency(fobValue) : <span className="text-slate-300">--</span>}
                      </td>
                      <td className="whitespace-nowrap px-5 py-3.5 text-sm font-medium text-slate-900">
                        {valorNumerario != null ? formatCurrency(valorNumerario, 'BRL') : <span className="text-slate-300">--</span>}
                      </td>
                      <td className="whitespace-nowrap px-5 py-3.5">
                        {percentual != null ? (
                          <span className={cn(
                            'inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold',
                            percentual >= 100 ? 'bg-emerald-50 text-emerald-700' :
                            percentual >= 50 ? 'bg-blue-50 text-blue-700' :
                      ...
                      <td className="whitespace-nowrap px-5 py-3.5 text-sm text-slate-500">
                        {getField(data, 'dataPgtoNumerario') ? formatDate(getField(data, 'dataPgtoNumerario')!) : <span className="text-slate-300">--</span>}
                      </td>
                      <td className="whitespace-nowrap px-5 py-3.5 text-sm text-slate-600">
                        {getField(data, 'solicitanteNumerario') || <span className="text-slate-300">--</span>}
                      </td>
                      <td className="max-w-[200px] truncate px-5 py-3.5 text-sm text-slate-500">
                        {getField(data, 'dadosCambio') || <span className="text-slate-300">--</span>}
                      </td>
                    </tr>
                  );
                })}
