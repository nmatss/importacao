...
function StatusDropdown({
  ...
  return (
    <div
      ref={dropdownRef}
      className="absolute top-full right-0 z-50 mt-1 w-56 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 shadow-lg py-1 max-h-72 overflow-y-auto"
    >
                ...
                {idx < LOGISTIC_STAGES.length - 1 && (
                  <div
                    className={cn(
                      ...
                      idx < currentStep ? 'bg-emerald-300' : 'bg-slate-200 dark:bg-slate-700',
                    )}
                  />
                )}
              </div>
