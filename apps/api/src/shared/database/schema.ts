...
export const sydlePurchasePayments = pgTable(
  'sydle_purchase_payments',
  {
    ...
    sydleProtocol: varchar('sydle_protocol', { length: 50 }),
    processCode: varchar('process_code', { length: 50 }),
    purchaseRef: varchar('purchase_ref', { length: 100 }),
    ...
    shipmentDate: date('shipment_date'),
    paymentDeadlineAfterShipment: integer('payment_deadline_after_shipment'),
    exceptionStatus: varchar('exception_status', { length: 50 }),
  ...
  (table) => [
    ...
    index('sydle_purchase_payments_protocol_idx').on(table.sydleProtocol),
    ...
    index('sydle_purchase_payments_shipment_date_idx').on(table.shipmentDate),
