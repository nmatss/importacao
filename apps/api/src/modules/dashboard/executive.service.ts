...
export const executiveService = {
  async getExecutiveKpis() {
    ...
    const [
      ...
      db
        ...
        .where(
          and(
            ...
            gte(importProcesses.updatedAt, lastMonthStart),
            sql`${importProcesses.updatedAt} < ${monthStartStr}`,
          ),
        ),
      // FOB this month
      db
      ...
      // Avg validation pass rate (only count passed vs failed, ignore warning/skipped)
      db
        .select({
