...
if not API_KEY:
    log.error("CERT_API_KEY is not set — protected cert-api routes will fail closed")
...
def _fetch_wms_stock():
    ...
    host = os.environ.get("WMS_ORACLE_HOST", "")
    port = int(os.environ.get("WMS_ORACLE_PORT", "1521"))
    sid = os.environ.get("WMS_ORACLE_SID", "")
    user = os.environ.get("WMS_ORACLE_USER", "")
    password = os.environ.get("WMS_ORACLE_PASS", "")
...
def _fetch_ecommerce_stock(brand: str):
    ...
    if brand.lower() in ('puket', 'puket_escolares'):
        host = os.environ.get("ERP_PUKET_HOST", "")
        db_name = os.environ.get("ERP_PUKET_DB", "")
        filial = "EXTREMA - MG"
        where = f"filial = '{filial}'"
    else:
        host = os.environ.get("ERP_IMG_HOST", "")
        db_name = os.environ.get("ERP_IMG_DB", "")
        where = "filial LIKE '%IMAGINARIUM EXTREMA MG%'"

    user = os.environ.get("ERP_MSSQL_USER", "")
